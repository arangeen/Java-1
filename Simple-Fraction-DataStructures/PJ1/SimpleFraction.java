...
public class SimpleFraction implements SimpleFractionInterface, Comparable<SimpleFraction>
{
	...
	public SimpleFraction(int num, int den)
	{
		// implement this method!
            /*
            if den is 0 then throw exception. if den is a negative number 
            then make sure to bring the negative up to the num. or else 
            let everything be.
	...
	public void simplifySimpleFraction()
      ...
      // making sure to put the negative sign always in the numerator of the fraction 
            if (den < 0)
                {
                    num = -1*num;
                    den = -1*den;
	...
	public double toDouble()
	{
		...
		double doubleValue = (double)(num)/(double)(den);
                return doubleValue;
	}	// end toDouble 
	...
	public SimpleFractionInterface add(SimpleFractionInterface secondFraction)
	{
		...
		// a/b + c/d is (ad + cb)/(bd)
            ...
            SimpleFraction a = new SimpleFraction(num, den);
            SimpleFraction b = (SimpleFraction) secondFraction;
            
            int topPart = (a.num*b.den)+(b.num*a.den);
            int bottomPart = (a.den*b.den);
            ...
            int topPartReduced = topPart / gcd;
            int bottomPartReduced = bottomPart / gcd;
            
            if (bottomPartReduced <0)
            {
                topPartReduced = -1*(topPartReduced);
                bottomPartReduced = -1*(bottomPartReduced);
                
            }
            a.setSimpleFraction(topPartReduced, bottomPartReduced);
	...
	public SimpleFractionInterface divide(SimpleFractionInterface secondFraction)
	{
		...
		// a/b / c/d is (ad)/(bc)
            ...
            {
                int topPart = (a.num*b.den);
                int bottomPart = (a.den*b.num);
                int gcd = topPart;
                int t1 = 0;
                int t2 = bottomPart;
                while (t2!=0)
	...
	public boolean equals(Object other)
	{
		// implement this method!
            SimpleFraction a = new SimpleFraction(num, den);
            SimpleFraction b = (SimpleFraction) other;
            ...
            int topPartReduced = (a.num / gcd);
            int bottomPartReduced = (a.den / gcd);
            
            int q1 = 0;
            int greatestCommonDenominator = b.num;
            int q2 = b.den;
            while (q2 != 0)
            {
                q1 = greatestCommonDenominator % q2;
                greatestCommonDenominator = q2;
                q2 = q1;
            }
            int topPartReduced2 = (a.num / gcd);
            int bottomPartReduced2 = (a.den / gcd);
            
            if (topPartReduced == topPartReduced2 && bottomPartReduced == bottomPartReduced2)
            {
                return true;
            }
            else
   ...
   //the value returned is 0 when the fractions are equal, 
     //   -1 if less than, 1 if greater than
